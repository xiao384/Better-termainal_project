...
#sudo 功能
    def sudo_permission(self,account):
        def sign_in_sudo():
            global root_permission,result_text
            result_text = None
            with open(os.path.join(__run_path__, 'users_data.json'), 'r', encoding='utf-8') as f:
                ...
                for i in range(len(list_name)):
                    if list_name[i] == 'root':
                        ...
                        break
                if root_pwd is not None and password_input == root_pwd:
                    root_permission = True
                    result_text = '成功获得root权限'
        ...
        if account == 'root':
            ...
            sudo_permission_tk.geometry('200x150')
            sudo_permission_tk.resizable(False, False)
            sudo_permission_tk_label = tk.Label(sudo_permission_tk,text='请输入root密码:')
            sudo_permission_tk_label.pack(pady=15)
            sudo_permission_tk_Entry = tk.Entry(sudo_permission_tk)
            sudo_permission_tk_Entry.pack(pady=15)
            sudo_permission_tk_Button = tk.Button(sudo_permission_tk,text='确认',command=sign_in_sudo)
            sudo_permission_tk_Button.pack(pady=15)
...
def main():
    ...
    if users_permission:
        sign_tk.destroy()
        main_tk = tk.Tk()
        main_tk.title(f'bash-{__system_name__}--{root.users_name}--{root.users_permission_detailed}')
